# copied from http://www.tjansson.dk/2012/11/yabi-yet-another-boids-implementation-simulation-of-flocking-animals/
...
def plot_raw():
    ...
    global ax  # works without the global keywords?
...
def plot_init():
    ...
    # returns like: array([[ 0.,  0.,  0.],
    #                      [ 0.,  0.,  0.]])
    boidpos = numpy.zeros((NumberOfBoids, Dimensions))
    predpos = numpy.zeros((NumberOfPredators, Dimensions))

    # returns like: [(0, boid_obj), (1, boid_obj)]
    for index,boid in list(enumerate(flock)):
        boidpos[index]= boid.pos
    bx = boidpos[:,0]
    by = boidpos[:,1]
    bz = boidpos[:,2]

    for index,predator in list(enumerate(predators)):
...
# returns like: [<boids_test.boid object at 0x101dcfc18>, <boids_test.boid object at 0x101e88208>]
...
anim = animation.FuncAnimation(fig, plot_update, init_func=plot_init,frames=timesteps, interval=20)
